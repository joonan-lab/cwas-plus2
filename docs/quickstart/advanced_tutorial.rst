...
############################################
  ...
  - **GENE_MATRIX**: This is the file name of the gene matrix, which is a tab-separated text file. It must have a column named ``gene_id`` (or ``ensembl_gene_id``) holding Ensembl gene IDs. This column is the key used to match genes, because gene symbols are not unique. A gene symbol column named ``gene_name`` (or ``gene_symbol``, or ``symbol``) is optional and is used only to label the output. Columns are found by name, ignoring case, so their order does not matter, and the matrix should carry exactly one gene ID column and at most one symbol column. Every remaining column represents a gene list and shows whether each row (=gene) is matched to the gene list or not by a binary code (0, 1). 1 if the gene is matched to a gene list, 0 if not. The five key column names are reserved, so a gene list must not be named after any of them: such a column is consumed as a key column and disappears from the categories with no warning. A version suffix on the gene ID (e.g. ``ENSG00000000003.15``) is ignored when genes are matched.
